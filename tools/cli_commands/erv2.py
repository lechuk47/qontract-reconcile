...
class TerraformCli:
    ...
    def _rds_import_password_set_keepers(
        ...
        rds_password_obj = self.resource_changes(
            TfAction.CREATE
        )._get_resource_by_address(rds_password_addr)
    ...
    def _rds_import_enhanced_monitoring_role(self, rds_name: str) -> None:
        ...
        em_role = next(
            (
                r
                for r in self.resource_changes(TfAction.CREATE)._get_resources_by_type(
                    "aws_iam_role"
                )
    ...
    def _remove_from_old_state(
        ...
        old_instance = old_resources._get_resource_by_address(rds_address)
        ...
        old_instance_pg = old_resources._get_resource_by_address(
        ...
        old_em_role = old_resources._get_resource_by_address(em_role_addr)
        if old_em_role:
        ...
        old_em_role_att = old_resources._get_resource_by_address(em_role_att_addr)
        if old_em_role_att:
    ...
    def migrate_rds(self, source: TerraformCli, identifier: str) -> None:
        ...
        if new_resources._get_resource_by_address(rds_address):
        ...
        if new_resources._get_resource_by_address(password_address):
            # import the password
            self._rds_import_password(source, rds_name)

        em_role_address = f"aws_iam_role.{rds_name}-enhanced-monitoring"
        if new_resources._get_resource_by_address(em_role_address):
